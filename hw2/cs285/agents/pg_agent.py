...
class PGAgent(BaseAgent):
    ...
    def train(self, observations, actions, rewards_list, next_observations, terminals):
        ...
        trajectory_num = len(rewards_list)
        train_log = self.actor.update(observations, actions, advantages, q_values, trajectory_num)
        return train_log
    ...
    def _discounted_return(self, rewards):
        ...
        T = len(rewards)
        sum = np.sum((self.gamma ** np.arange(T)) * np.asarray(rewards))
        # NOTE(youngsang): _discounted_return ouputs a list with every element as the sum value
        list_of_discounted_returns = sum.repeat(T)
    ...
    def _discounted_cumsum(self, rewards):
        ...
        for t in range(T):
            sum = np.sum((self.gamma ** np.arange(T-t)) * np.asarray(rewards[t:]))
            list_of_discounted_cumsums[t] = sum
